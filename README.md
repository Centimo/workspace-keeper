...
виртуальный десктоп + CLion + WezTerm (tmux) + Floorp (браузер).
...
- **config/** — конфигурации WezTerm, tmux, KDE, Floorp
...
│   ├── tmux/tmux.conf             # конфиг мультиплексора
│   ├── kde/
...
Терминальные сессии переживают перезагрузку через tmux + resurrect.
...
tmux-плагины (устанавливаются вручную в `~/.tmux/plugins/`):
